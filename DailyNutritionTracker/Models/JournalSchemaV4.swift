...
/// Frozen schema matching live types at Wave 4 — the shape on disk before per-entry macros.
///
/// Snapshotted rather than pointed at the live types (as it briefly was) so that adding
/// `macrosJSON` and the protein-grams goal in V5 is a real, describable migration step.
enum JournalSchemaV4: VersionedSchema {
    ...
    final class DailyLog {
        ...
        init() {
            ...
            self.workoutEntries = []
            self.feelingEntries = []
            self.checkedFatsAndVeggiesJSON = "[]"
            self.checkedFatsJSON = "[]"
            self.checkedMiscItemsJSON = "[]"
            self.checkedFruitsJSON = "[]"
            self.completedSupplementsJSON = "[]"
        }
    }
